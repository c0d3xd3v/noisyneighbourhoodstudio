...
# === Audio-Konfiguration (jetzt mit QMediaPlayer) ===
media_player = None          # wird später initialisiert
audio_output = None
current_audio_file = ""      # Pfad der aktuell geladenen Audiodatei
...
# Für den roten Cursor (jetzt nur noch eine Linie, kein eigener Timer mehr)
playback_line = None
...
audio_output.setVolume(0.8)   # 80 % Lautstärke
...
def on_click(plot, points):
    ...
    for p in points:
        ...
        current_audio_file = fname   # speichern für QMediaPlayer
...
def stop_playback():
    ...
    if media_player.playbackState() == QMediaPlayer.PlayingState:
        media_player.stop()
    # Signal trennen, damit kein Cursor-Update mehr kommt
    try:
        media_player.positionChanged.disconnect()
    except:
        pass
    ...
    if playback_line and playback_line in waveform_plot.items():
        ...
        playback_line = None
    info_label.setText(f"📁 Session: {os.path.basename(current_waveform_dir)} – {len(x_values)} Trigger")

# (Die Funktion load_session bleibt fast unverändert, nur dass stop_playback aufgerufen wird)
def load_session(session_path):
    ...
    waveform_plot.clear()
    spectrogram_plot.clear()
    stop_playback()          # laufende Wiedergabe beenden
    play_button.setEnabled(False)
    save_button.setEnabled(False)
...
# Am Ende: Wiedergabe stoppen beim Schließen (optional)
